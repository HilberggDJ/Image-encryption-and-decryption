...
Wersja finalna z logo UWB z pliku logo.png
"""
...
class ImageProcessor:
    ...
    def etap2_unscramble(img: np.ndarray, key: int) -> np.ndarray:
        ...
        perm = ImageProcessor._fisher_yates(n, seed=key)
        inv_perm = ImageProcessor._inverse_permutation(perm)
...
ETAPY = {
    ...
    "Etap 2 – Odczyt zig-zag + permutacja": (
...
def generate_demo_image(kind="checker", size=(320, 240)):
    ...
    if kind == "checker":
        ...
        draw.text(
            (20, 90),
            "Chaotyczne przekształcanie obrazu",
            fill=(100, 20, 20),
            font=small,
        )
        draw.text(
            (20, 125), "tekst / struktura / kontrast", fill=(20, 120, 70), font=small
        )
...
class App(tk.Tk):
    ...
    def _load_logo(self, size=(190, 58)):
        candidates = ["logo.png", "logo.jpg", "logo.jpeg", "logo.bmp"]
        ...
        try:
            ...
            font = ImageFont.load_default()
        draw.text((16, 18), "UWB Wilno", fill=(233, 239, 255, 255), font=font)
        return ImageTk.PhotoImage(img)
    ...
    def _build(self):
        ...
        ttk.Label(top, image=self.logo_img, style="Header.TLabel").pack(
            side=tk.LEFT, padx=(0, 12)
        )

        info = ttk.Frame(top)
        info.pack(side=tk.LEFT, padx=2)
        ttk.Label(info, text="Analizator etapów 1 / 2 / 3", style="Header.TLabel").pack(
            anchor=tk.W
        )
        ttk.Label(
            info,
            text="odczyt zig-zag · P i P^-1 · błędny klucz · metryki eksperymentalne",
            style="Sub.TLabel",
        ...
        self.status_var = tk.StringVar(
            value="Wczytaj obraz lub wygeneruj obraz testowy."
        )
        ttk.Label(self, textvariable=self.status_var, style="Sub.TLabel").pack(
            fill=tk.X, padx=18, pady=(0, 12)
        )
    ...
    def _build_left(self, parent):
        ...
        ttk.Label(
            wrap, text="Obraz", style="Panel.TLabel", font=("Segoe UI Semibold", 12)
        ).pack(anchor=tk.W)
        ttk.Button(wrap, text="Wczytaj obraz", command=self._load).pack(
            fill=tk.X, pady=(8, 6)
        )
        self.path_lbl = ttk.Label(
            wrap, text="(brak)", style="Panel.TLabel", wraplength=300
        )
        self.path_lbl.pack(anchor=tk.W, pady=(0, 10))

        ttk.Label(
            wrap,
            text="Szybki obraz testowy",
            style="Panel.TLabel",
        ...
        ttk.Button(
            demo_row, text="Szachownica", command=lambda: self._load_demo("checker")
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 4))
        ttk.Button(
            demo_row, text="Gradient", command=lambda: self._load_demo("gradient")
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=4)
        ttk.Button(
            demo_row, text="Tekst", command=lambda: self._load_demo("text")
        ).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(4, 0))

        ttk.Separator(wrap, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=10)
        ttk.Label(
            wrap, text="Etap", style="Panel.TLabel", font=("Segoe UI Semibold", 12)
        ).pack(anchor=tk.W)
        self.etap_var = tk.StringVar(value=list(ETAPY.keys())[0])
        ttk.Combobox(
            wrap,
            textvariable=self.etap_var,
            values=list(ETAPY.keys()),
            state="readonly",
        ...
        ttk.Button(
            wrap, text="Scramble", style="Accent.TButton", command=self._scramble
        ).pack(fill=tk.X, pady=4)
        ttk.Button(
            wrap,
            text="Unscramble – poprawny klucz",
            command=lambda: self._unscramble(True),
        ).pack(fill=tk.X, pady=4)
        ttk.Button(
            wrap,
            text="Unscramble – błędny klucz",
            command=lambda: self._unscramble(False),
        ).pack(fill=tk.X, pady=4)
        ttk.Button(
            wrap, text="Test formalny P⁻¹(P(i))", command=self._run_formal_table
        ).pack(fill=tk.X, pady=(10, 4))
        ttk.Button(wrap, text="Zapisz wyniki", command=self._save).pack(
            fill=tk.X, pady=(4, 4)
        )
        ...
        note = (
            "Etap 2: odczyt zig-zag + permutacja Fisher-Yates bez zmiany wartości pikseli.\n"
            "Etap 3: Etap 2 + XOR z maską PRNG, w pełni odwracalny.\n"
            "Testy: poprawny i błędny klucz, metryki, analiza formalna."
        )
        ttk.Label(
            wrap, text=note, style="Panel.TLabel", wraplength=300, foreground=C["muted"]
        ).pack(anchor=tk.W, pady=(6, 0))
    ...
    def _build_right(self, parent):
        ...
        for lbl in labels:
            ...
            ttk.Label(
                inner, text=lbl, style="Panel.TLabel", font=("Segoe UI Semibold", 12)
            ).pack(pady=(0, 8))
            c = tk.Canvas(
                inner,
                width=IMG_W,
            ...
            c.pack()
            info = ttk.Label(
                inner, text="—", style="Panel.TLabel", foreground=C["muted"]
            )
            info.pack(pady=(8, 0))
        ...
        self._set_text(
            self.formal_text, "Tu pojawi się tabela P(i), P^-1(i), P^-1(P(i)).\n"
        )
        self._set_text(
            self.report_text, "Tu pojawi się raport tekstowy do zapisania.\n"
        )
    ...
    def _load(self):
        ...
        self.status_var.set(
            f"Wczytano: {os.path.basename(path)} · {img.width}x{img.height} px"
        )
    ...
    def _get_key(self, correct=True):
        ...
        try:
            ...
            messagebox.showerror(
                "Błędny klucz", f"Klucz musi być liczbą całkowitą.\nWpisano: {raw}"
            )
            return None
    ...
    def _run_formal_table(self):
        try:
            ...
            messagebox.showerror(
                "Błędny klucz", "Klucz poprawny musi być liczbą całkowitą."
            )
            return
        n = 256
        ...
        for row in rows:
            lines.append(
                f"{row['i']:>3} | {row['P(i)']:>4} | {row['P^-1(i)']:>7} | {row['P^-1(P(i))']:>10}"
            )
        lines.append("")
    ...
    def _save(self):
        ...
        if report:
            with open(
                os.path.join(folder, "raport_metryki.txt"), "w", encoding="utf-8"
            ) as f:
                f.write(report)
